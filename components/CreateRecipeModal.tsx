...
const CreateRecipeModal: React.FC<CreateRecipeModalProps> = ({ isOpen, onClose, rawMaterials, fixedCosts, onSave, recipeToEdit }) => {
  ...
  const handleAddIngredient = () => {
    const quantityNum = parseFloat(ingredientQuantity);
    if (selectedMaterialId && quantityNum > 0) {
  ...
  const costBreakdown: CostBreakdown = useMemo(() => {
    ...
    const yieldQty = parseInt(productionYield) || 1;
    const laborPct = parseFloat(laborPercentage) || 0;
    const servicesPct = parseFloat(servicesPercentage) || 0;
  ...
  const handleSave = () => {
    const yieldNum = parseInt(productionYield) || 0;
          ...
          <div>
            ...
            <div className="space-y-4 bg-white p-4 rounded-lg border">
              <input type="text" placeholder="Nombre de la Receta" value={recipeName} onChange={e => setRecipeName(e.target.value)} className="w-full p-2 bg-white border rounded" />
               <div>
                  <label className="block text-sm font-medium text-slate-700">Unidades que rinde esta receta</label>
                  <input type="number" value={productionYield} onChange={e => setProductionYield(e.target.value)} className="w-full p-2 border rounded mt-1 bg-white" min="1" />
              </div>
            </div>
          </div>
          <div>
             <h3 className="font-bold text-lg mb-2">2. Ingredientes (para el lote completo)</h3>
            ...
            <div>
                 ...
                 <div className="bg-white p-4 rounded-lg border space-y-4">
                    <div>
                        ...
                        <input type="number" value={laborPercentage} onChange={e => setLaborPercentage(e.target.value)} className="w-full p-2 border rounded mt-1 bg-white" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-slate-700">Servicios (% del total de costos fijos)</label>
                        <input type="number" value={servicesPercentage} onChange={e => setServicesPercentage(e.target.value)} className="w-full p-2 border rounded mt-1 bg-white" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-slate-700">Ganancia (% de materias primas + servicios)</label>
                        <input type="number" value={profitPercentage} onChange={e => setProfitPercentage(e.target.value)} className="w-full p-2 border rounded mt-1 bg-white" />
                    </div>
                 </div>
            </div>
            <div>
                ...
                <div className="bg-indigo-50 p-4 rounded-lg border border-indigo-200 space-y-2">
                    ...
                    <div className="flex justify-between text-sm border-t pt-2 mt-2"><span className="text-slate-800 font-bold">Costo de Producción Total (lote de {productionYield} und)</span><span className="font-bold">${costBreakdown.costoTotalProduccion.toFixed(2)}</span></div>
